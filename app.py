...
TMDB_API_KEY = "c90fb79a2f7d756a49bee848bce5f413" # Sua chave TMDB para tradução
...
def obter_link_direto_ia(identifier):
    try:
        ...
        for f in r.get("files", []):
            n = f.get("name", "").lower()
            if n.endswith('.mp4'): return f"https://archive.org/download/{identifier}/{urllib.parse.quote(f.get('name'))}"
    except: pass
    return None
...
def buscar_alldebrid_pro(titulo, id_tmdb=""):
    """Tenta o nome em PT, se falhar, busca o nome Original no TMDB para o AllDebrid achar"""
    try:
        termos_busca = [titulo]
        
        # 1. Se falhar, tenta pegar o nome original no TMDB
        if id_tmdb:
            ...
            if nome_original and nome_original.lower() != titulo:
                termos_busca.append(nome_original.lower())
            ...
            if r_yts.get("data", {}).get("movies"):
                ...
                if r_up.get("status") == "success":
                    ...
                    if links:
                        ...
                        return requests.get(url_un, timeout=5).json().get("data", {}).get("link")
        return None
    except: return None
...
def buscar():
    ...
    # 🥈 2. ALLDEBRID (AGORA TRADUZINDO NOMES)
    ...
    if tmdb_id: return redirect(f"https://vidsrc.net/embed/movie?tmdb={tmdb_id}")
